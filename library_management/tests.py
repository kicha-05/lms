...
class TestBookView(APITestCase):
    ...
    def test_book_created(self):
        respone = self.client.post(self.url, data=self.book_data, format="json")
...
class TestAuthorView(APITestCase):
    ...
    def test_author_created(self):
        respone = self.client.post(self.url, data=self.author_data, format="json")
        self.assertEqual(respone.status_code, 201)
...
class TestCollectionView(APITestCase):
    ...
    def test_collection_created(self):
        respone = self.client.post(self.url, data=self.collection_data, format="json")
        self.assertEqual(respone.status_code, 201)
...
class TestSearchView(APITestCase):
    ...
    def test_book_search(self):
        respone = self.client.get(self.url, data={"q": "book", "keyword": "dummy"})
        self.assertEqual(respone.status_code, 200)
        self.assertEqual(respone.data["books"][0]["name"],self.book.name)
    
    def test_author_search(self):
        respone = self.client.get(self.url, data={"q": "author", "keyword": "dummyauthor"})
        self.assertEqual(respone.status_code, 200)
        self.assertEqual(respone.data["books"][0]["name"],self.book.name)
    
    def test_collection_search(self):
        respone = self.client.get(self.url, data={"q": "collection", "keyword": "dummycollection"})
        self.assertEqual(respone.status_code, 200)
        self.assertEqual(respone.data["books"][0]["name"],self.book.name)